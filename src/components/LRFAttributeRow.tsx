  ...
  return (
    <div className="bg-white border-b border-gray-100 last:border-b-0">
      <button
        ...
        className="flex w-full items-center justify-between px-4 py-3 pl-8 text-left hover:bg-slate-50 transition-colors cursor-pointer"
      >
        <div className="flex items-center gap-2.5">
          ...
          />
          <span className={`text-[13px] ${isCustom ? "italic text-[#1e2a52]" : "text-gray-600"}`}>
            {label}
            {isCustom && <span className="text-[#1e2a52]"> *</span>}
          </span>
        </div>
        {hasDefined && (
          <span className="text-[12px] font-bold text-gray-800 pr-2">{changeType}</span>
        )}
      </button>

      {isExpanded && (
        <div className="px-4 pb-3 pt-1 ml-8 mr-4 space-y-3 border-l-2 border-[#1e2a52]/20">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            <div className="flex-1">
              <label className="block text-xs font-medium text-gray-400 mb-1">Change type</label>
              <select
                value={changeType}
            ...
            {showOld && (
              <div className="flex-1">
                <label className="block text-xs font-medium text-gray-400 mb-1">Old value</label>
                <input
                  type="text"
                  value={oldValue}
            ...
            {showRegion && (
              <div className="flex-1">
                <label className="block text-xs font-medium text-gray-400 mb-1">Area / region</label>
                <input
                  type="text"
                  value={newValue}
                  onChange={(e) => onNewValue(e.target.value)}
            ...
            <button
              ...
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md text-gray-400 hover:bg-red-50 hover:text-red-600 transition-colors cursor-pointer"
              title="Clear"
            >
              <X size={14} />
            </button>
