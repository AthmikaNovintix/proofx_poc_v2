  ...
  return (
    <div className="border-b border-gray-200 last:border-b-0">
      <button
        ...
        className="flex w-full items-center justify-between px-4 py-3 bg-white hover:bg-slate-50 transition-colors border-b border-gray-100 cursor-pointer"
      >
        <div className="flex items-center gap-3">
          <FolderOpen size={16} className="text-[#1e2a52] stroke-[1.5]" />
          <span className="text-[14px] font-semibold text-gray-800">{name}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[13px] text-gray-500">{allAttrs.length} items</span>
          {definedCount > 0 && (
            <span className="text-[13px] font-bold text-gray-800">{definedCount} defined</span>
          )}
          <ChevronDown
            size={14}
